...
class HelperListAdminMpStockProduct extends HelperList
{
    ...
    private function getList()
    {
        ...
        if (!Tools::isSubmit('submitReset' . $this->table)) {
            ...
            $filter_local_date_document = Tools::getValue('local_' . $this->table . 'Filter_date_document', [0, 0]);
            if (!empty($filter_local_date_document[0]) || !empty($filter_local_date_document[1])) {
                $filter_date_document = Tools::getValue($this->table . 'Filter_date_document', [0, 0]);
            ...
            $filter_date_add = Tools::getValue($this->table . 'Filter_date_add', [0, 0]);
        ...
        $sql1->select('distinct p.id_mpstock_product as `id`')
            ...
            ->leftJoin('mpstock_document_v2', 'doc', 'doc.id_mpstock_document=p.id_document')
