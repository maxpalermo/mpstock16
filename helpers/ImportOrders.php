...
class ImportOrders
{
    ...
    public function importOrders($orders)
    {
        ...
        $orderDetails = $db->executeS($sql);

        $success = [];
        $errors = [];

        foreach ($orderDetails as $orderDetail) {
            $record = $this->getOrderDetail($orderDetail, 113);
    ...
    protected function getOrderDetail($orderDetail, $id_mvt)
    {
        $record = [
